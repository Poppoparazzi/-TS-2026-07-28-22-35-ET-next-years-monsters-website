...
test("Monster Rating batch prioritizes persisted readiness and SEC evidence depth before verified revenue", async () => {
  ...
  assert.ok(historyReadyOrder > pilotOrder, "persisted provider-backed history readiness must rank before ordinary SEC depth signals");
  assert.ok(factOrder > historyReadyOrder, "SEC fact depth must rank after persisted history readiness");
