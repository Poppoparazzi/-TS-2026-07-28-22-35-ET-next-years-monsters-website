...
test("Monster Rating batch prioritizes persisted readiness, fresh stored liquidity, and SEC evidence before verified revenue", async () => {
  ...
  assert.ok(storedLiquidityPresentOrder > historyReadyOrder, "fresh stored liquidity availability must rank after persisted history readiness");
  assert.ok(storedLiquidityValueOrder > storedLiquidityPresentOrder, "stronger stored dollar-volume evidence must rank ahead of SEC-depth tie breakers");
