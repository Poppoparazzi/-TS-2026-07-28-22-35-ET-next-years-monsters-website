...
function selectFact(
  ...
  const candidates = (summary.factHistory[key] ?? [])
    .filter(isAnnual)
...
export function buildAnnualFinancialPeriods(
  ...
  for (const candidate of summary.factHistory.revenue ?? []) {
  ...
  const periods = anchors.map<FinancialPeriodEvidence>((revenue) => {
    ...
    const grossProfit = selectFact(summary, "grossProfit", revenue, monetaryUnit);
    const operatingIncome = selectFact(summary, "operatingIncome", revenue, monetaryUnit);
    const netIncome = selectFact(summary, "netIncome", revenue, monetaryUnit);
    const dilutedEps = selectFact(summary, "dilutedEps", revenue, monetaryUnit);
    const assets = selectFact(summary, "assets", revenue, monetaryUnit);
    const liabilities = selectFact(summary, "liabilities", revenue, monetaryUnit);
    const shareholdersEquity = selectFact(
      summary,
      "shareholdersEquity",
      revenue,
      monetaryUnit,
    );
    const cash = selectFact(summary, "cash", revenue, monetaryUnit);
