...
const FACT_DEFINITIONS: readonly FactDefinition[] = Object.freeze([
  { key: "revenue", taxonomy: "us-gaap", tags: Object.freeze(["RevenueFromContractWithCustomerExcludingAssessedTax", "Revenues", "SalesRevenueNet"]) },
  { key: "grossProfit", taxonomy: "us-gaap", tags: Object.freeze(["GrossProfit"]) },
  { key: "operatingIncome", taxonomy: "us-gaap", tags: Object.freeze(["OperatingIncomeLoss"]) },
  { key: "netIncome", taxonomy: "us-gaap", tags: Object.freeze(["NetIncomeLoss", "ProfitLoss"]) },
  { key: "dilutedEps", taxonomy: "us-gaap", tags: Object.freeze(["EarningsPerShareDiluted"]) },
  { key: "assets", taxonomy: "us-gaap", tags: Object.freeze(["Assets"]) },
  { key: "liabilities", taxonomy: "us-gaap", tags: Object.freeze(["Liabilities"]) },
  { key: "shareholdersEquity", taxonomy: "us-gaap", tags: Object.freeze(["StockholdersEquity", "StockholdersEquityIncludingPortionAttributableToNoncontrollingInterest"]) },
  { key: "cash", taxonomy: "us-gaap", tags: Object.freeze(["CashAndCashEquivalentsAtCarryingValue", "CashCashEquivalentsRestrictedCashAndRestrictedCashEquivalents"]) },
  { key: "operatingCashFlow", taxonomy: "us-gaap", tags: Object.freeze(["NetCashProvidedByUsedInOperatingActivities"]) },
]);

const PERIODIC_FORMS = new Set(["10-K", "10-K/A", "10-Q", "10-Q/A", "20-F", "20-F/A", "40-F", "40-F/A", "6-K", "6-K/A"]);
...
function validEntry(entry: SecFactUnitEntry): boolean {
  return Boolean(validDate(entry.end) && validDate(entry.filed) && entry.accn && entry.form && PERIODIC_FORMS.has(entry.form) && typeof entry.val === "number" && Number.isFinite(entry.val));
}
...
function contextIdentity(snapshot: SecFactSnapshot): string {
  return [snapshot.unit, snapshot.periodStart ?? "", snapshot.periodEnd, snapshot.fiscalYear ?? "", snapshot.fiscalPeriod ?? "", snapshot.form.replace(/\/A$/, "")].join("|");
}

function conceptSnapshots(key: string, taxonomy: string, tag: string, concept: SecFactConcept, cik: number): readonly SecFactSnapshot[] {
  const snapshots: SecFactSnapshot[] = [];
  for (const [unit, entries] of Object.entries(concept.units ?? {})) {
    for (const entry of entries) {
      if (!validEntry(entry) || !entry.end || !entry.filed || !entry.accn || !entry.form) continue;
...
export function parseSecFactHistory(payload: SecCompanyFactsPayload, cik: number): ParsedSecFactHistory {
  ...
  for (const definition of FACT_DEFINITIONS) {
    ...
    for (const tag of definition.tags) {
      ...
      if (!concept) continue;
      for (const snapshot of conceptSnapshots(definition.key, definition.taxonomy, tag, concept, cik)) {
  ...
  return Object.freeze({ latest: Object.freeze(latest), history: Object.freeze(history) });
}
