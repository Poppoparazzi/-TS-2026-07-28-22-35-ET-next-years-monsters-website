...
(function installCurrentRatingReadiness() {
  ...
  function ensurePanel(result) {
    ...
    if (!document.getElementById("monster-current-readiness-styles")) {
      ...
      style.textContent = `
        ...
        .monster-current-readiness-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:10px}
    ...
    panel.innerHTML = `
      <div class="monster-current-readiness-head">
        <div>
          ...
          <h3>VERIFYING REQUIRED EVIDENCE</h3>
          <p class="monster-current-readiness-summary">Checking the machine-readable quote, freshness, financial, risk, and versioned-calculation paths. No missing component will be guessed or filled with demonstration data.</p>
        </div>
        <div class="monster-current-readiness-state">CHECKING…</div>
      </div>`;
  }
  ...
  function explicitTimestamp(data) {
  ...
  async function verify(ticker, result, generation) {
    ...
    const freshness = explicitTimestamp(quote.data);
    ...
    const checks = {
      quote: quote.ok && isFiniteNumber(price),
      freshness: quote.ok && Boolean(freshness),
      financial: fundamentals.ok && hasSubstantiveValue(financial),
      risk: analysis.ok && hasSubstantiveValue(risk),
      versioned: analysis.ok && isFiniteNumber(score) && hasSubstantiveValue(version),
      sec: sec.ok && hasSubstantiveValue(sec.data)
    };

    const requiredReady = checks.quote && checks.freshness && checks.financial && checks.risk && checks.versioned;
    ...
    panel.innerHTML = `
      <div class="monster-current-readiness-head">
        <div>
          ...
          <p class="monster-current-readiness-summary">${requiredReady
            ? `All required evidence gates responded with explicit supported values. Calculation version: ${safe(version)}. Quote timestamp: ${safe(freshness)}.`
            : "A current numeric rating is deliberately withheld until every required evidence gate is present. Historical VCL™ demonstration scores never substitute for missing current evidence."}</p>
        </div>
      ...
      <div class="monster-current-readiness-grid">
        ${evidenceItem("MARKET QUOTE", checks.quote, checks.quote ? `Explicit price received: ${price}` : `Missing explicit current price (HTTP ${quote.status || "offline"}).`)}
        ${evidenceItem("QUOTE FRESHNESS", checks.freshness, checks.freshness ? `Timestamp: ${freshness}` : "Missing explicit quote/trade timestamp.")}
        ${evidenceItem("FINANCIAL EVIDENCE", checks.financial, checks.financial ? "Explicit current financial field(s) received." : `Required financial evidence missing (HTTP ${fundamentals.status || "offline"}).`)}
        ${evidenceItem("RISK EVIDENCE", checks.risk, checks.risk ? "Explicit risk field(s) received." : `Required risk evidence missing (HTTP ${analysis.status || "offline"}).`)}
        ${evidenceItem("VERSIONED CALCULATION", checks.versioned, checks.versioned ? `Score ${score} · version ${version}` : "No score is accepted without an explicit calculation/model version.")}
      </div>
      <p class="monster-current-readiness-source">SEC identity cross-check: ${checks.sec ? "connected" : `not confirmed (HTTP ${sec.status || "offline"})`}. Required rating gates are quote + freshness + financial evidence + risk evidence + versioned calculation. Fail-closed means no manufactured number.</p>`;
  }
